...
function main()
    ...
    for left in 1:4
        for right in 1:4
            ...
            push!(stds,std(measure_res)/sqrt(parsed_args["samples"]))
        end
    end
    total_std = sqrt(sum([x^2 for x in stds]))
    sample_res["mean"] = mean_value
    sample_res["true_mean"] = real(true_mean_value)
    sample_res["std"] = total_std
    println("mean value: $mean_value")
    println("total std: $total_std")
    println("true mean value: $true_mean_value")
    file_name = "/Users/hyhu/Git_Code/Fermionic.jl/simple_Ham/simple_direct_measurement_data/L$(parsed_args["L"])_type$(parsed_args["type"])_dis$(parsed_args["distance"])_Delta$(parsed_args["Delta"])_mu$(parsed_args["mu"])_samples$(parsed_args["samples"]).json"
